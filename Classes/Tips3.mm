...
-(void)dosomething1
{
	...
	[XMLUtilView initialMyPraseror:[NSString stringWithFormat:@"otherinfo"] 
								  :[[NSMutableArray alloc]initWithObjects:@"name",@"image",@"content",@"detail",@"name1",nil]];
...
-(void)dosomething2
{
	...
	self.navigationItem.title = @"其他地區資訊";
	c_mun3=0;
	c_narray=[[NSMutableArray alloc]init];
	c_namenarray=[[NSMutableArray alloc]init];
	c_mumnarray=[[NSMutableArray alloc]init];
         ...
         for (int i=0; i<[c_mumnarray count]; i++) 
         {
             if (indexPath.section==i) 
             {
                 ...
                 if (i==0) 
                 {
                     ...
                     [c_image setImage:[UIImage imageWithContentsOfFile:[c pathForResource:[NSString stringWithFormat:@"%@",[[XMLUtilView.tmpdic objectForKey:@"image"]objectAtIndex:indexPath.row]] ofType:@""]]];
                     NSLog(@"%@",[[XMLUtilView.tmpdic objectForKey:@"image"]objectAtIndex:indexPath.row]);
                     ...
                     //c_image.frame=CGRectMake(0, 0, 90, 59);
                     c_image.backgroundColor=[UIColor clearColor];    
                     [cell.contentView addSubview:c_image];
                     [c_image release];
                     
                     NSString *x = [[XMLUtilView.tmpdic objectForKey:@"name1"]objectAtIndex:indexPath.row];
                     NSLog(@"gggg: %@", x);
                     UITextView *c_textview=[[UITextView alloc]initWithFrame:CGRectMake(70+50, 10, 260, 30)];
                     c_textview.text=x;
                     ...
                     x = [[XMLUtilView.tmpdic objectForKey:@"content"]objectAtIndex:indexPath.row];
                     UITextView *c_textview1=[[UITextView alloc]initWithFrame:CGRectMake(70+50, 32, 260, 30)];
                     c_textview1.text=x;
                   ...
                   //  c_textview1.alpha=0.0f;
                     c_textview1.font=[UIFont boldSystemFontOfSize:10];
                 ...
                 }             else 
                 {
                     ...
                     NSString *x = [[XMLUtilView.tmpdic objectForKey:@"name1"]objectAtIndex:indexPath.row+c_mun3];
                     NSLog(@"gggg: %@", x);
                     UITextView *c_textview=[[UITextView alloc]initWithFrame:CGRectMake(70+50, 10, 260, 60)];
                     c_textview.text=x;
                     [cell.contentView addSubview:c_textview];
                     c_textview.backgroundColor=[UIColor clearColor];
                     c_textview.userInteractionEnabled=NO;
                     ...
                     c_textview1.text=[[XMLUtilView.tmpdic objectForKey:@"content"]objectAtIndex:indexPath.row+c_mun3];
                     c_textview1.backgroundColor=[UIColor clearColor];
                     c_textview1.textColor=[UIColor grayColor];
                     c_textview1.userInteractionEnabled=NO;
                    // c_textview1.alpha=0.0f;
    ...
    for (int i=0; i<[c_mumnarray count]; i++) 
    {
        if (indexPath.section==i) 
        {
            if (i==0) 
            {
                ...
                Tips3_1 *Tips3=[[Tips3_1 alloc]initWithNibName:@"Tips3_1" bundle:nil];
                Tips3.c_tittle=[[NSString alloc]initWithFormat:@"%@",[[XMLUtilView.tmpdic objectForKey:@"name1"]objectAtIndex:indexPath.row]];
                //c_Tips2.c_xml=[[NSString alloc]initWithFormat:[[XMLUtilView.tmpdic objectForKey:@"detail"]objectAtIndex:indexPath.row]];
                Tips3.c_xml=nowdate;
                [self.navigationController pushViewController:Tips3 animated:YES];
                ...
                Tips3_1 *Tips3=[[Tips3_1 alloc]initWithNibName:@"Tips3_1" bundle:nil];
                Tips3.c_tittle=[[NSString alloc]initWithFormat:@"%@",[[XMLUtilView.tmpdic objectForKey:@"name1"]objectAtIndex:indexPath.row+c_mun3]];
