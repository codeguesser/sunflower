...
-(UIViewController *) createWebViewController: (NSString *) urlString{
	...
	//NSString *c_new=[[NSString alloc]initWithFormat:@"aboutus/%@",c_xml];
...
//		[webView loadRequest: request];
		...
		self.navigationItem.title = @"熱線電話指南";
		UIViewController * webViewController = [[UIViewController alloc] init];
		UIWebView * webView = [[UIWebView alloc] initWithFrame:CGRectMake(0, -45, 320, 368+45)];
		webView.delegate=self;
		NSURL *url = [NSURL URLWithString: urlString];
		NSString *body = nil;
		NSMutableURLRequest *request = [[NSMutableURLRequest alloc]initWithURL: url];
...
- (UITableViewCell *)tableView:(UITableView *)c_tv
	...
	if (cell == nil) {
		...
		// Set up the cell...
		//int x = indexPath.row;
		cell.imageView.image =  [UIImage imageNamed:@"cruise.png"]; //gaurav
